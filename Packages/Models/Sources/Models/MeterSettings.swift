...
public extension MeterSettings {
    ...
    struct MeterTime: Equatable, Codable, Sendable {
        ...
        public var seconds: TimeInterval { return TimeInterval((hour * 60 * 60) + (minute * 60)) }
    }
}

public extension MeterSettings.MeterTime {

    func toMeterDateTime(
        for sourceDate: Date = .now,
        in calendar: Calendar
    ) -> Date {
        guard let dateWithMeterTime = calendar.date(bySettingHour: hour, minute: minute, second: 0, of: sourceDate) else {
