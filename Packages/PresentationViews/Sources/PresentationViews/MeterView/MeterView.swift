...
public struct MeterView: View {
    ...
    @Binding public var accumulatedEarningsDateSelection: Date
    
    private var isEnabled: Bool { reading.progress > 0 }
    private var showsDatePicker: Bool { style == .accumulated }
    private var today: Date { calendar.startOfDay(for: .now) }
    private var isTodaySelected: Bool { calendar.isDateInToday(accumulatedEarningsDateSelection) }
    ...
    public init(style: Style,
                ...
                accumulatedEarningsDateSelection: Binding<Date>) {
        self.style = style
        self.settings = settings
        self.reading = reading
        self._accumulatedEarningsDateSelection = accumulatedEarningsDateSelection
    }
    ...
    private var meterHeader: some View {
        Group {
            ...
            case .accumulated:
                Text("meter.header.earnings.since.title \(accumulatedEarningsDateSelection.formatted(date: .abbreviated, time: .omitted))", bundle: .module)
            }
        }
        .textCase(.uppercase)
        .font(.title2)
    ...
    private var datePicker: some View {
        DatePicker(selection: $accumulatedEarningsDateSelection,
                   in: ...today,
    ...
    private var dateResetButton: some View {
        Button {
            accumulatedEarningsDateSelection = .now
        } label: {
            Text("Reset")
        }
...
private struct WrapperView: View {
    ...
    @State var accumulatedEarningsDateSelection: Date = .now
